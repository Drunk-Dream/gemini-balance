...
async def verify_openai_api_key(authorization: Optional[str] = Header(None)):
    ...
    if not authorization.startswith("Bearer "):
        raise HTTPException(
            status_code=401,
            detail="Invalid Authorization header format. Must be 'Bearer YOUR_API_KEY'",
        )
...
async def create_chat_completion_endpoint(
    ...
    logger.info(
        f"Received OpenAI chat completion request for model: {request.model}, stream: {request.stream}"  # noqa: E501
    )
